  ...
  if (!project) {
    return (
      <WorkspaceEmptyState
        eyebrow="设计书确认"
        title="没有找到这个项目"
        description="这个确认页对应的项目不存在，可能是链接失效，或者项目还没有创建成功。"
        actions={[
  ...
  if (!brief) {
    return (
      <WorkspaceEmptyState
        ...
        actions={[
          { href: `/workspace/${project.id}/design`, label: "返回设计书预览" },
          { href: `/workspace/${project.id}`, label: "返回工作区", variant: "secondary" },
        ]}
      />
    );
      ...
      <div className="container-shell space-y-8">
        <PageBackLinks
          items={[
            ...
            { label: "返回上一步", type: "back", fallbackHref: `/workspace/${project.id}/design` },
          ]}
        />
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(300px,1fr)]">
          ...
          <aside className="space-y-5">
            ...
            <WorkspaceQuickLinks
              title="继续推进"
              description="确认当前摘要无误后，直接进入任务执行。如果你想先回看设计书或项目信息，也可以从这里跳转。"
              links={[
                { href: `/workspace/${project.id}/tasks`, label: "确认并进入任务执行", variant: "primary" },
